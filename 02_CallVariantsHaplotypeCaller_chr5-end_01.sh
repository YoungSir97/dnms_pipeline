...
mkdir ${pathTMP}${i}_5_end/
pathTMP=${pathTMP}${i}_5_end/
...
gatk --spark-runner LOCAL --java-options "-Djava.io.tmpdir=${pathTMP}" HaplotypeCaller \
    ...
    --output ${pathVCF}${i}_mapped_fixmate_sorted_raw_snps_indels_chr5_end.g.vcf && \
	#--output ${pathVCF}${i}_mapped_fixmate_sorted_bqsr_raw_snps_indels_chr1_4.g.vcf
# -----------------------------------------------------------------------------------
ENDTIME=$(date +%s) && \
