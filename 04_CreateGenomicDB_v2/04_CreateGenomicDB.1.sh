...
pathTMP="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/script/04_CreateGenomicDB/tmp1/"
LIST="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/script/04_CreateGenomicDB/interval1.list"
...
gatk --java-options "-Djava.io.tmpdir=${pathTMP}" GenomicsDBImport \
   ...
   --genomicsdb-workspace-path ${pathVCF}AllTrioGenome.db1 && \

echo "finish CreateGenomicDB" && \

gatk --java-options "-Djava.io.tmpdir=${pathTMP}" GenotypeGVCFs \
   --reference $REF \
   -V gendb://${pathVCF}AllTrioGenome.db1 \
   --output ${pathVCF}ALLTrios_raw_snps_indels1.vcf && \
