...
pathTMP="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/script/04_CreateGenomicDB/tmp2/"
LIST="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/script/04_CreateGenomicDB/interval2.list"
...
gatk --spark-runner LOCAL --java-options "-Djava.io.tmpdir=${pathTMP}" GenomicsDBImport \
   ...
   --genomicsdb-workspace-path ${pathVCF}AllTrioGenome.db2 && \

echo "finish CreateGenomicDB" && \

gatk --java-options "-Djava.io.tmpdir=${pathTMP}" GenotypeGVCFs \
    --reference $REF \
    -V gendb://${pathVCF}AllTrioGenome.db2 \
    --output ${pathVCF}ALLTrios_raw_snps_indels2.vcf && \
