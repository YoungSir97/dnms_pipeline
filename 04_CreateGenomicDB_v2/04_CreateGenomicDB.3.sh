...
pathTMP="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/script/04_CreateGenomicDB/tmp3/"
LIST="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/script/04_CreateGenomicDB/interval3.list"
...
gatk --spark-runner LOCAL --java-options "-Djava.io.tmpdir=${pathTMP}" GenomicsDBImport \
   ...
   --genomicsdb-workspace-path ${pathVCF}AllTrioGenome.db3 && \

echo "finish CreateGenomicDB" && \

gatk --java-options "-Djava.io.tmpdir=${pathTMP}" GenotypeGVCFs \
    --reference $REF \
    -V gendb://${pathVCF}AllTrioGenome.db3 \
    --output ${pathVCF}ALLTrios_raw_snps_indels3.vcf && \
