...
inputSNP="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/vcf/07_FilterRepeat/ALLTrios_raw_SNP_MaskedRepeat.vcf.gz"
inputINDEL="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/vcf/07_FilterRepeat/ALLTrios_raw_INDEL_MaskedRepeat.vcf.gz"
# output path and prefix
outputPath="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/vcf/08_HardFilter/"
outNameSNP="" # If not set, the input file name will be used.
outNameINDEL="" # If not set, the input file name will be used.
# report file output path
pathREPORT="/home/ubuntu/1.Project/DNMs/02.process.oldgenome/vcf/08_HardFilter/report/"
...
# other parameters
SNPparameters='MQ < 40.0 || QD < 2.0 || FS > 20.0 || MQRankSum < -2.000 || MQRankSum > 4.000 || ReadPosRankSum < -3.000 || ReadPosRankSum > 3.000'
INDELparameters='QD < 2.0 || FS > 200.0 || QUAL < 30.0 ||ReadPosRankSum < -20.000'
...
## filter SNPs with hard filter criteria (MQ < 40.0 || QD < 2.0 || FS > 20.0 || MQRankSum < -2.000 || MQRankSum > 4.000 || ReadPosRankSum < -3.000 || ReadPosRankSum > 3.000)
if [ $inputSNP ]; then
    # check input
    ...
    gatk --spark-runner LOCAL --java-options "-Djava.io.tmpdir=${pathTMP}" VariantFiltration \
        -R $REF \
        -V ${inputSNP} \
        --filter-expression "$SNPparameters" \
    ...
    Rscript ${pathSCRIPT}/plot_SampleDepth.R -i ${outputPath}/dp_SNPs.tmp -s 20 -o ${pathREPORT}/08_HardFilter_Samples_SNP && \
...
if [ $inputINDEL ]; then
    ...
    gatk --spark-runner LOCAL --java-options "-Djava.io.tmpdir=${pathTMP}" VariantFiltration \
        -R $REF \
        -V ${inputINDEL} \
        --filter-expression "$INDELparameters" \
    ...
    Rscript ${pathSCRIPT}/plot_SampleDepth.R -i ${outputPath}/dp_INDELs.tmp -s 20 -o ${pathREPORT}/08_HardFilter_Samples_INDEL && \
...
if [ $inputSNP ] && [ $inputINDEL ]; then
    ...
    Rscript ${pathSCRIPT}/plot_VariantScores.R -s ${outputPath}/${outNameSNP}_hardfilterPASS.table -i ${outputPath}/${outNameINDEL}_hardfilterPASS.table -o ${pathREPORT}/08_HardFilter_VariantsScores.pdf
    ...
    Rscript ${pathSCRIPT}/plot_VariantScores_SNP.R -s ${outputPath}/${outNameSNP}_hardfilterPASS.table -o ${pathREPORT}/08_HardFilter_VariantsScores_SNP.pdf
elif [ ! $inputSNP ] && [ $inputINDEL ]; then
    [ -e ${pathSCRIPT}/plot_VariantScores_INDEL.R ] || { echo "script does not exist: ${pathSCRIPT}/plot_VariantScores_INDEL.R"; exit 1 ;}
    Rscript ${pathSCRIPT}/plot_VariantScores_INDEL.R -i ${outputPath}/${outNameINDEL}_hardfilterPASS.table -o ${pathREPORT}/08_HardFilter_VariantsScores_INDEL.pdf
fi && \

\rm -r $pathTMP && \
# -----------------------------------------------------------------------------------
## combine True Variant
bcftools concat -a ${outputPath}/${outNameSNP}_hardfilterPASS.vcf.gz ${outputPath}/${outNameINDEL}_hardfilterPASS.vcf.gz \
            --output-type z -o ${outputPath}/TRUESITES.vcf.gz && \
gatk IndexFeatureFile -I ${outputPath}/TRUESITES.vcf.gz && \
