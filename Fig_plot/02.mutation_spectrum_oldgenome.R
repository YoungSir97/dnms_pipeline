...
dt <- read.csv("./DNMs_information_list_oldgenome.csv",header = T)
head(dt)

p1 <- ggplot(dt, aes(x=type3,fill=CpGsite,alpha=CpGsite)) +
...
p1
ggsave("./突变谱1_oldgenome.pdf",plot = p1,width = 6,height = 5)


p2 <- ggplot(dt, aes(x=type3,fill=Source)) +
...
p2
ggsave("./突变谱2_oldgenome.pdf",plot = p2,width = 7,height = 5)
...
p3 <- ggplot(dt,aes(x=Source,fill=Source,alpha = CpGsite))+
  ...
  ggtitle("de novo mutation spectrum")+
  facet_wrap(~type3,nrow=1,ncol=6)+
  scale_fill_manual(values=c("#f8766d","#00b0f6","grey")) +
  scale_alpha_manual(values=c(0.3,1))+
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5,face = "bold",size=17))  #设置标题居中
p3
ggsave("./突变谱3_oldgenome.pdf",plot = p3,width = 7,height = 5)
