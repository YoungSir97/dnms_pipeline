...
if (is.null(opt$output)){
    opt$output <- "plot_VariantScores.pdf"
}
...
DP <- ggplot(VCF, aes(x=DP, fill=Variant)) + geom_density(alpha=0.3)
  #geom_vline(xintercept=c(25,100)) + xlim(0,2000)

QD <- ggplot(VCF, aes(x=QD, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=2,colour=snps) +
  geom_text(aes(x=2, y=0, label=2),colour=snps, size=3, vjust=1.5, hjust=1.2)

FS <- ggplot(VCF, aes(x=FS, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=c(60,200), colour = c(snps,indels)) + xlim(0,200) +
  geom_text(aes(x=60, y=0,label=60),colour=snps, size=3, vjust=1.5, hjust=1.2) +
  geom_text(aes(x=200, y=0,label=200),colour=indels, size=3, vjust=1.5, hjust=1.2)

MQ <- ggplot(VCF, aes(x=MQ, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=40, colour=snps) +
  geom_text(aes(x=40, y=0, label=40),colour=snps, size=3, vjust=1.5, hjust=1.2)

MQRankSum <- ggplot(VCF, aes(x=MQRankSum, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=-12.5, colour = snps) + xlim(-15,15) +
  geom_text(aes(x=-12.5, y=0, label=-12.5),colour=snps, size=3, vjust=1.5, hjust=1.2)

SOR <- ggplot(VCF, aes(x=SOR, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=3, colour = snps) + xlim(0,10) +
  geom_text(aes(x=3, y=0, label=3),colour=snps, size=3, vjust=1.5, hjust=1.2)

ReadPosRankSum <- ggplot(VCF, aes(x=ReadPosRankSum, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=-8, colour = snps) + xlim(-10, 10) +
  geom_text(aes(x=-8, y=0, label=-8),colour=snps, size=3, vjust=1.5, hjust=1.2)
