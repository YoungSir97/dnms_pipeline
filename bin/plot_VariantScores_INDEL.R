...
arg <- matrix(c("input", "i","1","character","input INDELs table file",
                "output","o","1","character","output",
                "height","l","1","integer","image height,default=20",
                "width","w","1","integer","image width,default=15",
                "help","h","0","logical", "Usage: Rscript 12_plot_VariantScores_SNP.R -i <indel.table> [-o output]"),byrow=T,ncol=5)
opt = getopt(arg)
...
if (is.null(opt$output)){
    opt$output <- "plot_VariantScores_INDEL.pdf"
}
...
SOR_cutoff = 3
ReadPosRankSum_cutoff = -20

# read table
VCF <- read.csv(opt$input, header = T, na.strings=c("","NA"), sep = "\t")
#dim(VCF)
VCF$Variant <- factor(rep("INDELs", dim(VCF)[1]))
snps <- '#00bfff'
...
QD <- ggplot(VCF, aes(x=QD, fill=Variant)) + geom_density(alpha=.3) +
  ...
  geom_text(aes(x=QD_cutoff, y=0, label=QD_cutoff),colour=snps, size=3, vjust=1.5, hjust=1.2)

FS <- ggplot(VCF, aes(x=FS, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=FS_cutoff,colour=snps) + xlim(0,200) +
  geom_text(aes(x=FS_cutoff, y=0, label=FS_cutoff),colour=snps, size=3, vjust=1.5, hjust=1.2)

MQ <- ggplot(VCF, aes(x=MQ, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=MQ_cutoff,colour=snps) +
  geom_text(aes(x=MQ_cutoff, y=0, label=MQ_cutoff),colour=snps, size=3, vjust=1.5, hjust=1.2)
...
MQRankSum <- ggplot(VCF, aes(x=MQRankSum, fill=Variant)) + geom_density(alpha=.3) +
  ...
  geom_text(aes(x=MQRankSum_cutoff, y=0, label=MQRankSum_cutoff),colour=snps, size=3, vjust=1.5, hjust=1.2)

SOR <- ggplot(VCF, aes(x=SOR, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=SOR_cutoff,colour=snps) + xlim(0,10) +
  geom_text(aes(x=SOR_cutoff, y=0, label=SOR_cutoff),colour=snps, size=3, vjust=1.5, hjust=1.2)

ReadPosRankSum <- ggplot(VCF, aes(x=ReadPosRankSum, fill=Variant)) + geom_density(alpha=.3) +
  geom_vline(xintercept=ReadPosRankSum_cutoff,colour=snps) + xlim(-20, 20) +
  geom_text(aes(x=ReadPosRankSum_cutoff, y=0, label=ReadPosRankSum_cutoff),colour=snps, size=3, vjust=1.5, hjust=1.2)
