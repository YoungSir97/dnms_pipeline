...
foreach my $fam ( sort keys %trios){
    ...
    ## chrZ
    print SH1 "gatk --spark-runner LOCAL --java-options \"-Djava.io.tmpdir=\${pathTMP}\" SelectVariants -V $vcf --intervals chrZ -select 'vc.getGenotype(\"$t[2]\").isHomRef() && vc.getGenotype(\"$t[1]\").isHomRef() && vc.getGenotype(\"$t[0]\").isHet()' --exclude-filtered true -O $output1/$fam\_$t[0]_$t[1]_$t[2].tmp.vcf.gz && \\\nbcftools view -s $t[0],$t[1],$t[2] $output1/$fam\_$t[0]_$t[1]_$t[2].tmp.vcf.gz --output-type z -o $output1/$fam\_$t[0]_$t[1]_$t[2].GT.vcf.gz && \\\n\\rm $output1/$fam\_$t[0]_$t[1]_$t[2].tmp.vcf.gz && \\\n";
    print SH1 "bcftools stats $output1/$fam\_$t[0]_$t[1]_$t[2].GT.vcf.gz > $output1/$fam\_$t[0]_$t[1]_$t[2].GT.vcf.gz.stat.txt\n\n";

    if ($DPlist){
        #There is only one chrZ in the mother, and the DP cutoff is half.
        print SH2 "gatk --spark-runner LOCAL --java-options \"-Djava.io.tmpdir=\${pathTMP}\" IndexFeatureFile -I $output1/$fam\_$t[0]_$t[1]_$t[2].GT.vcf.gz && \\\ngatk --spark-runner LOCAL --java-options \"-Djava.io.tmpdir=\${pathTMP}\" SelectVariants -V $output1/$fam\_$t[0]_$t[1]_$t[2].GT.vcf.gz -select 'vc.getGenotype(\"$t[2]\").getDP()*2 >=$D{$t[2]}{1} && vc.getGenotype(\"$t[1]\").getDP() >=$D{$t[1]}{1} && vc.getGenotype(\"$t[0]\").getDP() >=$D{$t[0]}{1} && vc.getGenotype(\"$t[2]\").getDP()*2 <=$D{$t[2]}{2} && vc.getGenotype(\"$t[1]\").getDP() <=$D{$t[1]}{2} && vc.getGenotype(\"$t[0]\").getDP() <=$D{$t[0]}{2}' --exclude-filtered true -O $output2/$fam\_$t[0]_$t[1]_$t[2].GT.DP.vcf.gz && \\\nbcftools stats $output2/$fam\_$t[0]_$t[1]_$t[2].GT.DP.vcf.gz > $output2/$fam\_$t[0]_$t[1]_$t[2].GT.DP.vcf.gz.stat.txt\n\n";
    }else{
        warn "check\n";
...
print SH3 "echo \"Finish Step3.\"\n";
print SH4 "echo \"Finish Step4.\"\n";
print SH5 "echo \"Finish Step5.\"\n";
