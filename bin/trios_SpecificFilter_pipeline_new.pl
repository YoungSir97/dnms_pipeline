...
=head1 Example
    ...
    (Then, `cd output` and `bash step0.sh` ... )
=cut
use strict;
use Cwd 'abs_path';
use Getopt::Long;
...
my $output0 = "$outdir/Step0_SelectTrio_output";
open SH0,">$outdir/Step0_SelectTrio.sh" || die $!;
print SH0 "#!/bin/bash\n\n";
print SH0 "mkdir -p $output0\npathTMP=\"$tmpdir\"\n\n";
...
foreach my $fam ( sort keys %trios){
    ...
    print SH0 "bcftools view -s $t[0],$t[1],$t[2] $vcf -a --output-type z -o $output0/$fam\_$t[0]_$t[1]_$t[2].vcf.gz && \\\nbcftools stats $output0/$fam\_$t[0]_$t[1]_$t[2].vcf.gz > $output0/$fam\_$t[0]_$t[1]_$t[2].vcf.gz.stat.txt\n\n";

    print SH1 "gatk --spark-runner LOCAL --java-options \"-Djava.io.tmpdir=\${pathTMP}\" SelectVariants -V $output0/$fam\_$t[0]_$t[1]_$t[2].vcf.gz -select 'vc.getGenotype(\"$t[2]\").isHomRef() && vc.getGenotype(\"$t[1]\").isHomRef() && vc.getGenotype(\"$t[0]\").isHet()' --exclude-filtered true -O $output1/$fam\_$t[0]_$t[1]_$t[2].GT.vcf.gz && \\\n";
